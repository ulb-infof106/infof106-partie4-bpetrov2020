...
class Action:
    """
    Une instance de la classe Action représente une action faite par un joueur sur le plateau.
    ...
    def __init__(self, old_pos, new_pos, arrow_pos, player_id):
        ...
        if self.player not in (PLAYER_1, PLAYER_2):
            raise InvalidActionError('Une action doit être associée à un joueur valide')
    ...
    def as_Pos2D(pos):
        if isinstance(pos, str):
            ...
            raise InvalidActionError('Type inconnu pour un Pos2D: {type(pos)}')
